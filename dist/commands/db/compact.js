...
exports.compact = cli_1.Command
    .make('compact', { follow, databases }, ({ follow, databases }) => index_1.initializeUrl.pipe(effect_1.Effect.andThen(() => effect_1.Array.map(databases, compact_1.CompactService.compactDb)), effect_1.Effect.flatMap(effect_1.Effect.all), effect_1.Effect.map(effect_1.Option.liftPredicate(() => follow)), effect_1.Effect.map(effect_1.Option.map(core_1.mergeArrayStreams)), effect_1.Effect.map(effect_1.Option.map(compact_2.streamActiveTasks)), effect_1.Effect.flatMap(effect_1.Option.getOrElse(() => effect_1.Console.log('Compaction started. Watch the active tasks for progress: chtx active-tasks -f')))))
