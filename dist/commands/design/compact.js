...
const database = Args
    .text({ name: 'database' })
    .pipe(Args.withDescription('The database with the design to compact'));
const designs = Args
    .text({ name: 'design' })
    .pipe(Args.withDescription('The design(s) to compact'), Args.atLeast(1));
...
export const compact = Command
    .make('compact', { follow, database, designs }, Effect.fn(({ follow, database, designs }) => initializeUrl.pipe(Effect.andThen(CompactService.compactDesign(database)), Effect.map(compactDesign => Array.map(designs, compactDesign)), Effect.flatMap(Effect.allWith({ concurrency: 'unbounded' })), Effect.map(Option.liftPredicate(() => follow)), Effect.map(Option.map(mergeArrayStreams)), Effect.map(Option.map(streamActiveTasks)), Effect.flatMap(Option.getOrElse(() => Console.log('Compaction started. Watch the active tasks for progress: chtx active-tasks -f'))))))
