...
const printAllDesignDocNames = dbs_info_1.CouchDbsInfoService
    .getDbNames()
    .pipe(effect_1.Effect.flatMap(dbNames => (0, effect_1.pipe)(effect_1.Array.map(dbNames, design_docs_1.CouchDesignDocsService.getNames), effect_1.Effect.all, effect_1.Effect.map(effect_1.Array.zip(dbNames)), effect_1.Effect.map(getDisplayDict))), effect_1.Effect.map(d => JSON.stringify(d, null, 2)), effect_1.Effect.flatMap(effect_1.Console.log));
