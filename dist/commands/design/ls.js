...
const printDesignDocNames = Effect.fn((dbName) => getDesignDocNames(dbName)
...
export const ls = Command
    .make('ls', { database }, Effect.fn(({ database }) => initializeUrl.pipe(Effect.andThen(Effect.succeed(database)), Effect.map(Option.map(printDesignDocNames)), Effect.flatMap(Option.getOrElse(() => printAllDesignDocNames)))))
