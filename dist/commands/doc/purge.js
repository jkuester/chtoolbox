...
export const purge = Command
    .make('purge', { contacts, database, yes, all, reports, before, since }, (opts) => initializeUrl.pipe(Effect.andThen(assertOpts(opts)), Effect.andThen(isPurgeConfirmed(opts)), Effect.map(confirmed => Option.liftPredicate(purgeDocs(opts), () => confirmed)), Effect.flatMap(Option.getOrElse(() => Console.log('Operation cancelled')))))
