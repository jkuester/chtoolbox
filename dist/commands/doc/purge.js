...
const assertOpts = (opts) => effect_1.Match
    .value(opts)
    .pipe(effect_1.Match.when(({ all, contacts, reports }) => [all, reports, effect_1.Option.isSome(contacts)]
    .filter(Boolean)
    .length > 1, () => effect_1.Effect.fail(new Error('Cannot specify one of --all --contacts, and --reports'))), effect_1.Match.when(({ before, since, reports }) => !reports && (effect_1.Option.isSome(before) || effect_1.Option.isSome(since)), () => effect_1.Effect.fail(new Error('Can only specify --before or --since when using --reports'))), effect_1.Match.orElse(() => effect_1.Effect.void));
...
exports.purge = cli_1.Command
    .make('purge', { contacts, database, yes, all, reports, before, since }, (opts) => index_1.initializeUrl.pipe(effect_1.Effect.andThen(assertOpts(opts)), effect_1.Effect.andThen(confirmPurge(opts)), effect_1.Effect.flatMap(confirmed => (0, effect_1.pipe)(effect_1.Option.liftPredicate(purgeDocs(opts), () => confirmed), effect_1.Option.getOrElse(() => effect_1.Console.log('Operation cancelled'))))))
