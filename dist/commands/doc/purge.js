...
const clearLog = (...args) => effect_1.Console.clear.pipe(effect_1.Effect.tap(effect_1.Console.log(...args)));
const purgeAllDocs = (dbName) => purge_1.PurgeService
    .purgeAll(dbName)
...
}))), effect_1.Effect.map(effect_1.Stream.tap(({ current, total }) => clearLog(`Purging ${current.toString()}/${total.toString()}`))), effect_1.Effect.flatMap(effect_1.Stream.run(effect_1.Sink.last())), effect_1.Effect.map(effect_1.Option.getOrThrow), effect_1.Effect.tap(({ total }) => clearLog(`Purged ${total.toString()}/${total.toString()}`)));
...
exports.purge = cli_1.Command
    .make('purge', { database, yes }, ({ database, yes }) => index_1.initializeUrl.pipe(effect_1.Effect.andThen(confirmPurge(database, yes)), effect_1.Effect.flatMap(confirmed => (0, effect_1.pipe)(effect_1.Option.liftPredicate(purgeAllDocs(database), () => confirmed), effect_1.Option.getOrElse(() => effect_1.Console.log('Operation cancelled'))))))
