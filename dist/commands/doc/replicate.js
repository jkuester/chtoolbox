...
const printReplicatingDocs = (id) => Effect.fn((tasks) => pipe(tasks, Array.findFirst(isRepTask(id)), Option.map(({ docs_written }) => docs_written?.toString() ?? ''), Option.map(docs_written => clearThen(Console.log(`Replicating docs: ${docs_written}`))), Option.getOrElse(() => Effect.void), Effect.tap(Effect.logDebug('Printed replication doc task'))));
const streamReplicationTasks = Effect.fn((id) => streamActiveTasks()
    .pipe(Stream.tap(printReplicatingDocs(id)), Stream.runDrain));
const getReplicationDocId = Effect.fn((completionStream) => Stream
    .take(completionStream, 1)
    .pipe(Stream.runHead, Effect.map(Option.getOrThrow), Effect.map(({ _id }) => _id)));
const getFinalDocCount = Effect.fn((repDocId) => PouchDBService
    .get('_replicator')
...
const watchReplication = Effect.fn((completionStream) => getReplicationDocId(completionStream).pipe(Effect.flatMap(repDocId => Stream
...
export const replicate = Command
    .make('replicate', { follow, contacts, source, target, all }, Effect.fn(({ follow, contacts, source, target, all }) => initializeUrl.pipe(Effect.andThen(ReplicateService.replicate(source, target, {
    includeDdocs: all,
