...
export const printInstanceTable = Effect.fn((instances) => Match
...
const printUrls = Effect.fn((instances) => pipe(instances, Array.map(getLocalIpUrlBasicAuth), Array.map(Option.map(Console.log)), Array.map(Option.getOrElse(() => Effect.void)), Effect.all));
const print = (names) => Match
    .value(Array.isEmptyArray(names))
    .pipe(Match.when(true, () => printInstanceTable), Match.orElse(() => Effect.fn((instances) => pipe(instances, Array.filter(filterInstances(names)), printUrls))));
const names = Args
    .text({ name: 'name' })
