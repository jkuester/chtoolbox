...
export const warmViews = Command
    .make('warm-views', { follow }, Effect.fn(({ follow }) => pipe(initializeUrl, Effect.tap(Console.log('Warming views...')), Effect.andThen(WarmViewsService.warmAll()), Effect.andThen(() => followIndexing.pipe(Option.liftPredicate(() => follow), Option.getOrElse(() => Console.log('View warming started. Watch the active tasks for progress: chtx active-tasks -f')))))))
