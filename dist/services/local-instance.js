...
const createLocalVolumeDirsAtPath = Effect.fn((localVolumePath) => pipe(Array.make(SUB_DIR_CREDENTIALS, SUB_DIR_COUCHDB, SUB_DIR_NOUVEAU, SUB_DIR_DOCKER_COMPOSE), Array.map(subDir => `${localVolumePath}/${subDir}`), Array.map(createDir), Effect.allWith({ concurrency: 'unbounded' })));
const createLocalVolumeDirs = Effect.fn((localVolumePath) => localVolumePath.pipe(Option.map(path => assertLocalVolumeEmpty(path)), Option.map(Effect.flatMap(createLocalVolumeDirsAtPath)), Option.getOrElse(() => Effect.void)));
