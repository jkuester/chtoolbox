...
const assertLocalIpContainerDoesNotExist = Effect.fn((toPort) => doesContainerExist(getLocalIpContainerName(toPort))
    .pipe(Effect.filterOrFail(exists => !exists, () => new Error(`Local-ip instance already exists for port [${toPort.toString()}].`))));
const getFromPort = Effect.fn((fromPort) => fromPort.pipe(Option.map(port => getFreePort({ port })
...
const createLocalIpContainer = (toPort) => Effect.fn((fromPort) => runContainer({
...
const pullLocalIpImage = Effect.fn(() => pullImage(NGINX_LOCAL_IP_IMAGE)
...
export class LocalIpService extends Effect.Service()('chtoolbox/LocalIpService', {
    effect: serviceContext.pipe(Effect.map(context => ({
        create: Effect.fn((toPort, fromPort) => assertLocalIpContainerDoesNotExist(toPort)
        ...
        rm: Effect.fn((toPort) => rmContainer(getLocalIpContainerName(toPort))
            .pipe(Effect.mapError(x => x), Effect.provide(context))),
        ls: Effect.fn(() => getContainerNamesWithLabel(CHTX_LOCAL_IP_LABEL)
