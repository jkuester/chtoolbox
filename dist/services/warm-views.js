...
const warmCouchView = (dbName, designId) => Effect.fn((viewName) => warmView(dbName, designId, viewName));
...
const isDesignUpdating = Effect.fn((dbName, designId) => getDesignInfo(dbName, designId).pipe(Effect.tap(({ view_index }) => Effect.logDebug(`${dbName}/${designId} updater_running: ${view_index.updater_running.toString()}`)), Effect.map(({ view_index }) => view_index.updater_running)));
const warmDesignViews = Effect.fn((dbName, designId) => Effect
...
const isWarm = Effect.fn((dbName, designId) => Effect.all([
...
export class WarmViewsService extends Effect.Service()('chtoolbox/WarmViewsService', {
    effect: serviceContext.pipe(Effect.map(context => ({
        warmAll: Effect.fn(() => warmAll()
            .pipe(Effect.provide(context))),
        designsCurrentlyUpdating: Effect.fn(() => designsCurrentlyUpdating()
            .pipe(Effect.provide(context))),
