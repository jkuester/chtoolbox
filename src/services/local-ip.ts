...
const assertLocalIpContainerDoesNotExist = Effect.fn((
  toPort: number
...
const getFromPort = Effect.fn((fromPort: Option.Option<number>) => fromPort.pipe(
...
const createLocalIpContainer = (toPort: number) => Effect.fn((fromPort: number) => runContainer({
...
export class LocalIpService extends Effect.Service<LocalIpService>()('chtoolbox/LocalIpService', {
  effect: serviceContext.pipe(Effect.map(context => ({
    ...
    rm: Effect.fn((toPort: number): Effect.Effect<void, Error> => rmContainer(getLocalIpContainerName(toPort))
      .pipe(
        Effect.mapError(x => x as unknown as Error),
        Effect.provide(context),
      )),
    ls: Effect.fn((): Effect.Effect<{ from: number, to: number }[], Error> => getContainerNamesWithLabel(
      CHTX_LOCAL_IP_LABEL
    )
      .pipe(
